...
export async function postNewActivity(req: Request, res: Response) {
  ...
  const timeConflict = userActivities.find((activity: { time: string | number | Date | dayjs.Dayjs; duration: number; }) => {
    //se o dia for igual e a hora for igual
    if (dayjs(activity.time).format("YYYY-MM-DD HH:mm:ss") === dayjs(time).format("YYYY-MM-DD HH:mm:ss")) return true;
    //se a atividade a ser cadastrada durar mais de uma hora e a atividade já cadastrada começar em uma dessas horas seguintes
    if (duration > 1 && dayjs(activity.time).format("YYYY-MM-DD HH:mm:ss") === dayjs(time).add(duration - 1, "hour").format("YYYY-MM-DD HH:mm:ss")) return true;
    //se a atividade já cadastrada durar mais de uma hora e a atividade a ser cadastrada começar em uma dessas horas seguintes
    if (activity.duration > 1 && dayjs(activity.time).format("YYYY-MM-DD HH:mm:ss") === dayjs(time).subtract(activity.duration - 1, "hour").format("YYYY-MM-DD HH:mm:ss")) return true;
  });
